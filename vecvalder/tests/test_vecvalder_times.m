function [ok, funcname] = test_vecvalder_times(n, m, tol)
%function [ok, funcname] = test_vecvalder_times(n, m, tol)
%
%run tests on vecvalder/times; compare with handcoded expectations.
%
...
%Author: JR, 2014/06/18
    ...
    % if h(u,v) = u*v, dh_dx = v*du_dx + u*dv_dx
    expected_hv = oof.*poof;
    expected_hJ = (poof*ones(1,m)).*oof2 + (oof*ones(1,m)).*poof2;
    ok = ok && (max(abs(hv-expected_hv)) <= tol); 
    if ~(1==ok) 
        return; 
    end
