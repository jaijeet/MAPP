...
typedef Sacado::Fad::DFad<double>   F;  // FAD with # of ind. vars given later
	//      ^^^
	//   JR: forward automatic differentiation - USE THIS
	//   JR: D => dynamic => no. of indep. vars to be decided at runtime, not compile time; slower
typedef Sacado::Fad::SFad<double,2> F2; // FAD with # of ind. vars fixed at 2
	//   JR: S => static => no. of indep. vars decided at compile time => faster
typedef Sacado::Rad::ADvar<double>  R;  // for RAD
	//      ^^^
	//   JR: reverse automatic differentiation - NOBODY USES IT (according to Eric)

template <typename T> const T func2(T &a, T &b)	{ // sample function of 2 variables
	return sqrt(a*a + 2*b*b); 
}

template <typename T> const T func(int n, T *x)	{ // sample function of n variables
						  // == func2 when n == 2
	int i;
	T t = 0;
	for(i = 1; i < n; i++)
...
void Fad_demo() {
	...
	// JR: JUST DON'T USE FASTACCESS - SOMETIMES DOESN'T WORK, BE SAFE (Tom R.)

	// Similar exercise with general n, in this case n == 5
	n = 5;
	for(i = 0; i < n; i++) {
		x[i] = i;
		x[i].diff(i, n); // setting the independent variables
		}
	y = func(n, x);
	...
	// JR: TALK TO ERIC PHIPPS ABOUT GETTING a JACOBIAN STAMP OUT OF SACADO
}
