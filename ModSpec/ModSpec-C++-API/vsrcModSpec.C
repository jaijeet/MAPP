...
// constructor
vsrcModSpec::vsrcModSpec() {
	// model_name
	model_name = "Voltage source";
...
template <typename TOUT, typename TX, typename TY, typename TU>
  vector<TOUT> vsrcModSpec::fe_tmpl(vector<TX>& vecX, vector<TY>& vecY, vector<TU>& vecU) {
	vector<TOUT> out;
	TU E = vecU[0];
	TOUT vpn = E;
	out.push_back(vpn);
...
template <typename TOUT, typename TX, typename TY>
  vector<TOUT> vsrcModSpec::qe_tmpl(vector<TX>& vecX, vector<TY>& vecY) {
	vector<TOUT> out;
	out.push_back(0);
	return out;
}
...
template <typename TOUT, typename TX, typename TY, typename TU>
  vector<TOUT> vsrcModSpec::fi_tmpl(vector<TX>& vecX, vector<TY>& vecY, vector<TU>& vecU) {
	vector<TOUT> out;
	return out;
}

template <typename TOUT, typename TX, typename TY>
  vector<TOUT> vsrcModSpec::qi_tmpl(vector<TX>& vecX, vector<TY>& vecY) {
	vector<TOUT> out;
	return out;
}
