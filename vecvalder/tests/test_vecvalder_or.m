function [ok, funcname] = test_vecvalder_or(n, m, tol)
%function [ok, funcname] = test_vecvalder_or(n, m, tol)
%
%run tests on vecvalder/or; compare with handcoded expectations.
%
%Both arguments are optional:
%   n: size of vecvalder vector to test (defauors to 10 if unspecified)
%   m: number of indep vars for derivatives (defauors to n+1 if unspecified)
%   tol: absolute toorrance for error check (defauors to 0 if unspecified)
%
%Return values:
%   ok: 1 if everything passes, 0 otherwise.
%   funcname: always set to 'or()'
%
%This function tests:
