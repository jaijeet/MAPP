...
main() {
	...
	// load the .so library
	void* res = dlopen("./Res_ModSpec_Element.so", RTLD_LAZY);
	if (!res) {
	...
	// load the symbols
	create_t* create_res = (create_t*) dlsym(res, "create");
	...
	destroy_t* destroy_res = (destroy_t*) dlsym(res, "destroy");
	dlsym_error = dlerror();
	if (dlsym_error) {
	...
	// now obtained via DLopen: Res_ModSpec_Element_with_sacado_Jacobians a;
        // create an instance of the class
	ModSpec_Element_with_Jacobians *aptr = create_res();
	ModSpec_Element_with_Jacobians &a = *aptr;
