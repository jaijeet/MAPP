function MOD = MVS_1_0_1_3_ModSpec(uniqID)
% fetlim is added, but this time to vdib and vsib instead of vdb and vsb
% This doesn't make too much sense since vddi and vssi shouldn't be affected
% by limiting, but this is a quick way to see whether fetlim will work.
% f is separated
...
function [iddi, idisi, isis, qgb, qdib, qsib] = daa_mosfet_core_model(vd, vg, vs, vb, vdi, vsi, MOD)
    ...
    Vtpcorr = Vt0 + Gamma * (sqrt(smoothabs(phib - Vbs, smoothing))- sqrt(phib))- Vdsi * delta; % Calculated from extrinsic Vbs
    ...
    Vt0bs   = Vt0 + Gamma * (sqrt( smoothabs( phib - Vbscorr, smoothing)) - sqrt( phib ));      % Computed from corrected intrinsic Vbs
    Vt0bs0  = Vt0 + Gamma * (sqrt( smoothabs( phib - Vbsi, smoothing)) - sqrt( phib ));         % Computed from uncorrected intrinsic Vbs
    ...
    if (Gamma == 0)
	    ...
	    a = 1.0 + Gamma/ ( 2.0 * sqrt( smoothabs( psis - ( Vbsi ), smoothing)));
	end
    ...
    % DD-NVSAT charge
    Fsatq = smoothabs( Vdsi/ Vdsatq, smoothing )/ ( pow( 1.0 + pow( smoothabs( Vdsi/ Vdsatq, smoothing ), Beta ), 1.0/ Beta ));
    x     = 1.0 - Fsatq;
    den   = 15 * ( 1 + x ) * ( 1 + x );
    ...
    Qb = -Type * W * Leff * ( Cg * Gamma * sqrt( smoothabs( psis - Vbsi,smoothing )) + ( a - 1.0 )/ ( 1.0 * a ) * Qinv * ( 1.0 - qi ));
    ...
    Vt0x = Vt0 + Gamma * ( sqrt( smoothabs( phib - Type * ( vbsi ), smoothing)) - sqrt(phib));
    Vt0y = Vt0 + Gamma * ( sqrt( smoothabs( phib - Type * ( vbdi ), smoothing)) - sqrt(phib));
