...
%
    ...
    switch f2_switch
    case 1
        MOD = add_to_ee_model(MOD, 'parms', {'mu_v', 1e5});
        if 1 ~= f1_switch && 2 ~= f1_switch
            MOD = add_to_ee_model(MOD, 'parms', {'Ron', 20});
        end
    case 2
        MOD = add_to_ee_model(MOD, 'parms', {'a', 1e4, 'm', 5});
    case 3
    ...
    MOD = add_to_ee_model(MOD, 'parms', {'smoothing', 1e-10, 'Kclip', 50});
...
function out = fe(S)
    ...
    case 4
        if vpn>=0
            f1 = A1 * s * safesinh(B * vpn, maxslope);
        else % if vpn<0
            f1 = A2 * s * safesinh(B * vpn, maxslope);
        end
    case 5
...
function out = fi(S)
    ...
    case 5
        if vpn - (-Vn*s + Vp*(1-s)) >= 0
            g_of_vpn = Ap * (safeexp(vpn, maxslope) - safeexp(-Vn*s + Vp*(1-s), maxslope));
        else % if vpn - (-Vn*s + Vp*(1-s)) < 0
            g_of_vpn = -An * (safeexp(-vpn, maxslope) - safeexp(+Vn*s - Vp*(1-s), maxslope));
        end
