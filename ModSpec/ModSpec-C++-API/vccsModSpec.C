...
// constructor
vccsModSpec::vccsModSpec() {
	// model_name
	model_name = "Voltage-controlled current source";
	...
	explicit_output_names += "ipcnc", "ipnc", "innc";
...
template <typename TOUT, typename TX, typename TY, typename TU>
  vector<TOUT> vccsModSpec::fe_tmpl(vector<TX>& vecX, vector<TY>& vecY, vector<TU>& vecU) {
	TX vpnc = vecX[0];
	TX vnnc = vecX[1];
	TX vpcnc = vecX[2];
	double gain = this->parm_vals[0];
	vector<TOUT> out;
	out += 0, gain*vpcnc, -gain*vpcnc;
	return out;
}

template <typename TOUT, typename TX, typename TY>
  vector<TOUT> vccsModSpec::qe_tmpl(vector<TX>& vecX, vector<TY>& vecY) {
	vector<TOUT> out;
	out += 0, 0, 0;
	return out;
}
...
template <typename TOUT, typename TX, typename TY, typename TU>
  vector<TOUT> vccsModSpec::fi_tmpl(vector<TX>& vecX, vector<TY>& vecY, vector<TU>& vecU) {
	vector<TOUT> out;
	return out;
}

template <typename TOUT, typename TX, typename TY>
  vector<TOUT> vccsModSpec::qi_tmpl(vector<TX>& vecX, vector<TY>& vecY) {
	vector<TOUT> out;
	return out;
}
