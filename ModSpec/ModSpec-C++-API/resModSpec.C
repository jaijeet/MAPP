...
template <typename TOUT, typename TX, typename TY, typename TU>
  vector<TOUT> resModSpec::fe_tmpl(vector<TX>& vecX, vector<TY>& vecY, vector<TU>& vecU) {
	vector<TOUT> out;
	TX vpn = vecX[0];
...
template <typename TOUT, typename TX, typename TY>
  vector<TOUT> resModSpec::qe_tmpl(vector<TX>& vecX, vector<TY>& vecY) {
	vector<TOUT> out;
	out.push_back(0);
	return out;
}
...
template <typename TOUT, typename TX, typename TY, typename TU>
  vector<TOUT> resModSpec::fi_tmpl(vector<TX>& vecX, vector<TY>& vecY, vector<TU>& vecU) {
	vector<TOUT> out;
	return out;
}

template <typename TOUT, typename TX, typename TY>
  vector<TOUT> resModSpec::qi_tmpl(vector<TX>& vecX, vector<TY>& vecY) {
	vector<TOUT> out;
	return out;
}
