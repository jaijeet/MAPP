...
cccsModSpec::cccsModSpec() {
	...
	explicit_output_names += "vpcnc", "ipnc", "innc";
...
template <typename TOUT, typename TX, typename TY, typename TU>
  vector<TOUT> cccsModSpec::fe_tmpl(vector<TX>& vecX, vector<TY>& vecY, vector<TU>& vecU) {
	TX vpnc = vecX[0];
	TX vnnc = vecX[1];
	TX ipcnc = vecX[2];
	double gain = this->parm_vals[0];
	vector<TOUT> out;
	out += 0, gain*ipcnc, -gain*ipcnc;
	return out;
}

template <typename TOUT, typename TX, typename TY>
  vector<TOUT> cccsModSpec::qe_tmpl(vector<TX>& vecX, vector<TY>& vecY) {
	vector<TOUT> out;
	out += 0, 0, 0;
	return out;
}
...
template <typename TOUT, typename TX, typename TY, typename TU>
  vector<TOUT> cccsModSpec::fi_tmpl(vector<TX>& vecX, vector<TY>& vecY, vector<TU>& vecU) {
	vector<TOUT> out;
	return out;
}

template <typename TOUT, typename TX, typename TY>
  vector<TOUT> cccsModSpec::qi_tmpl(vector<TX>& vecX, vector<TY>& vecY) {
	vector<TOUT> out;
	return out;
}
