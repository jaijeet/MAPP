function [ok, funcname] = test_vecvalder_and(n, m, tol)
%function [ok, funcname] = test_vecvalder_and(n, m, tol)
%
%run tests on vecvalder/and; compare with handcoded expectations.
%
%Both arguments are optional:
%   n: size of vecvalder vector to test (defauands to 10 if unspecified)
%   m: number of indep vars for derivatives (defauands to n+1 if unspecified)
%   tol: absolute toandrance for error check (defauands to 0 if unspecified)
%
%Return values:
%   ok: 1 if everything passes, 0 otherwise.
%   funcname: always set to 'and()'
%
%This function tests:
