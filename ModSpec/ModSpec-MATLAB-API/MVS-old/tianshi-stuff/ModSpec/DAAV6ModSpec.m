...
% version, help string: 
	MOD.version = 'DAAV6ModSpec';
	MOD.Usage = help('DAAV6ModSpec');
	%
...
function dZf_dvecX = dfe_dvecX(vecX, vecY, vecLim, vecU, MOD)
	[Zf, dZf_dvecX] = fqei_dfqeidXYU(vecX, vecY, vecU, MOD, 'f', 'e', 'X');
end 
% end function dfe_dvecX

function dZf_dvecY = dfe_dvecY(vecX, vecY, vecLim, vecU, MOD)
	[Zf, dZf_dvecY] = fqei_dfqeidXYU(vecX, vecY, vecU, MOD, 'f', 'e', 'Y');
end 
% end function dfe_dvecY

function dZf_dvecU = dfe_dvecU(vecX, vecY, vecLim, vecU, MOD)
	dZf_dvecU = sparse(3,0);
end 
% end function dfe_dvecU

function dZq_dvecX = dqe_dvecX(vecX, vecY, vecLim, MOD)
	[Zq, dZq_dvecX] = fqei_dfqeidXYU(vecX, vecY, [], MOD, 'q', 'e', 'X');
end 
% end function dqe_dvecX

function dZq_dvecY = dqe_dvecY(vecX, vecY, vecLim, MOD)
...
function dWf_dvecX = dfi_dvecX(vecX, vecY, vecLim, vecU, MOD)
	[Wf, dWf_dvecX] = fqei_dfqeidXYU(vecX, vecY, vecU, MOD, 'f', 'i', 'X');
end 
% end function dfe_dvecX

function dWf_dvecY = dfi_dvecY(vecX, vecY, vecLim, vecU, MOD)
	[Wf, dWf_dvecY] = fqei_dfqeidXYU(vecX, vecY, vecU, MOD, 'f', 'i', 'Y');
end 
% end function dfe_dvecY

function dWf_dvecU = dfi_dvecU(vecX, vecY, vecLim, vecU, MOD)
	dWf_dvecU = sparse(2,0);
end 
% end function dfe_dvecU

function dWq_dvecX = dqi_dvecX(vecX, vecY, vecLim, MOD)
	[Wq, dWq_dvecX] = fqei_dfqeidXYU(vecX, vecY, [], MOD, 'q', 'i', 'X');
end 
% end function dqe_dvecX

function dWq_dvecY = dqi_dvecY(vecX, vecY, vecLim, MOD)
...
function [vecX, vecY] = initGuess(u, MOD)
...
% TODO: this is only the "backup" initialization of spice.
% Should get the real thing.	
	vbs = -1;
	vgs = Vt0;
	vds = 0;
	vecX(1, 1) = vds-vbs;	% vdb
	vecX(2, 1) = vgs-vbs;	% vgb
	vecX(3, 1) = -vbs;		% vsb
	vecY(1, 1) = vecX(1, 1);	% vdi_b
	vecY(2, 1) = vecX(3, 1);	% vsi_b
% vdi_b = vecY(1), vsi_b = vecY(2)
end % initGuess

%%%%%%%%%%%%%%%%% NR LIMITING SUPPORT %%%%%%%%%%%%%%%%%%%%%%
function [vecXnew, vecYnew] = limiting...
...
% type is not used here, but applied upstairs in
	...
	vbsold = -vsbold;
	vbdold = -vdbold;
	% TODO: vt and vcrit are hard-coded here
	% should get them somehow from parms
	vt = 0.026;
	vcrit = 0.6145;
	
	if vdsold >= 0
		vgs = fetlim(vgs, vgsold, Vt0);
		vds = vgs - vgd;
		vds = limvds(vds, vdsold);
		vgd = vgs - vds;
	else
		vgd = fetlim(vgd, vgdold, Vt0);
		vds = vgs - vgd;
		vds = -limvds(-vds, -vdsold);
		vgs = vgd + vds;
	end
	...
	vecXnew(3, 1) = -vbs;
	vecYnew = vecY;
	% TODO: internal variables not limited
end % limiting
