function MOD = cccsModSpec(uniqID)
%function MOD = cccsModSpec(uniqID)
%This function returns a ModSpec model for ideal current-controlled current
%sources.
%
%Arguments:
% - uniqID: (optional) a unique identification string. Eg, 'F1'.
%
%Return values:
% - MOD:    a ModSpec object for a current-controlled current source.
...
% - explicit output name(s) (vecZ):  vpcnc, ipnc, innc
% - other IO name(s) (vecX):         vpnc, vnnc, ipcnc
...
%    vpcnc = 0
%    gain*ipcnc-ipn = 0  -->  ipnc = gain*ipcnc
%    innc = - ipnc = ipn
% - fe = [0;
%         gain*ipcnc;
%         -gain*ipcnc];
...
%Examples
%--------
% % adding a cccs with gain equal to 0.5
% cktdata = add_element(cktdata, cccsModSpec(), 'F1', ...
...
% uniqID
    ...
    MOD.model_name = 'cccs';
    MOD.model_description = 'ideal current controlled current source';

    MOD.parm_names = {'gain'};
    MOD.parm_defaultvals = {1};
    MOD.parm_vals = MOD.parm_defaultvals; % current values of parms

    MOD.explicit_output_names = {'vpcnc', 'ipnc', 'innc'};
