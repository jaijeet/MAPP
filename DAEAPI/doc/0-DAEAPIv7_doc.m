...
%        x _must_ be a _column_ vector.
%
%DAE.fq (function handle). Use:
%if DAE.f_takes_inputs == 0, then:
%   [outf, outq] = feval(DAE.fq, x, u, flag, DAE);
%else if DAE.f_takes_inputs == 1, then:
%   [outf, outq] = feval(DAE.fq, x, flag, DAE);
% - if flag.f is 1, outf is a column vector of size neqns, evaluating DAE.f.
% - if flag.q is 1, outq is a column vector of size neqns, evaluating DAE.q.
%
%Requirement: return values of .fq must be consistent with those of f/q.
%
...
%
%DAE.fqJ (function handle). Use:
%if DAE.f_takes_inputs == 0, then:
%   fqJout = feval(DAE.fqJ, x, u, flag, DAE);
%else if DAE.f_takes_inputs == 1, then:
%   fqJout = feval(DAE.fqJ, x, flag, DAE);
%fqJout is a structure with the following fields:
% - if flag.f is 1, fqJout.f is a column vector of size neqns, evaluating DAE.f.
% - if flag.q is 1, fqJout.q is a column vector of size neqns, evaluating DAE.q.
% - if flag.dfdx is 1, fqJout.dfdx is a neqns-by-nunks matrix, evaluating DAE.df_dx.
% - if flag.dqdx is 1, fqJout.dqdx is a neqns-by-nunks matrix, evaluating DAE.dq_dx.
% - if flag.dfdu is 1, fqJout.dfdu is a neqns-by-ninputs matrix, evaluating DAE.df_du.
%
%Requirement: return values of .fqJ must be consistent with those of
...
%
%DAE.set_uQSS (function handle). Use:
...
%        feval(DAE.fq, x, u, flag, DAE) <--> feval(DAE.fq, x, xlim, u, flag, DAE);
%    if DAE.f_takes_inputs == 0
%        feval(DAE.f, x, DAE) <--> feval(DAE.f, x, xlim, DAE)
%        feval(DAE.fq, x, flag, DAE) <--> feval(DAE.fq, x, xlim, flag, DAE);
