%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%	This documents the functions in the DAEAPIv6.2+delta API
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%The DAE system with state x, inputs u(t) and outputs y(t), parameters p,
...
%		- to specify an input in the DAE itself, set DAE.uQSSvec in DAE's constructor.
% 	- for transient: DAE.utransient (set up by DAE.set_utransient) is used.
%		- to specify an input in the DAE itself, set DAE.utfunc and DAE.utargs in DAE's constructor.
% 	- for SSS/AC: DAE.uLTISSS (set up by DAE.set_uLTISSS) is used.
%		- to specify an input in the DAE itself, set DAE.uffunc and DAE.ufargs in DAE's constructor.
% 	- for HB (one tone): DAE.uHB (set up by DAE.set_uHB) is used.
%		- to specify an input in the DAE itself, set DAE.uHBfunc and DAE.uHBargs in DAE's constructor.
%	- unless you incorporate the inputs as an additional argument to f() - ie, f_takes_inputs == 1 -
%	  you will also need to define B(DAE) correctly to couple the input to the DAE. 
...
%DAE.renameUnks (function handle). Use: DAE = feval(DAE.renameUnks, oldnames_cellarray, newnames_cellarray, DAE);
...
%
%
%DAE.renameEqns (function handle). Use: DAE = feval(DAE.renameEqns, oldnames_cellarray, newnames_cellarray, DAE);
...
%DAE.renameParms (function handle). Use: DAE = feval(DAE.renameParms, oldnames_cellarray, newnames_cellarray, DAE);
...
%
%DAE.set_utransient (function handle). Use: DAE = feval(DAE.set_utransient, utfunc, utargs, DAE);
...
%
%DAE.set_uLTISSS (function handle). Use: DAE = feval(DAE.set_uLTISSS, Uffunc, Ufargs, DAE);
...
%
%DAE.set_uHB (function handle). Use: DAE = feval(DAE.set_uHB, uHBfunc, uHBargs, DAE);
