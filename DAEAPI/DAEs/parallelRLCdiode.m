...
%function DAE = parallelRLCdiode(uniqIDstr)   
% Current source parallel to parallel RLC diode [ I(t) || R || L || C || diode ] 
%author: J. Roychowdhury, 2009/11/08; updates 2011/11/05
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The Circuit:
% 
% [ I(t) || R || L || C || diode ] from node n1 to gnd
%
% unknowns are: e1, iL
% MNA equations are:
%
...
function out = daename(DAE)
	out = sprintf('parallelRLCdiode: I(t) || R || L || C || diode');
% end daename()
...
function fout = f(x, DAE)
	...
	%	C de1/dt + e1/R + iL + diode(e1,Is,Vt) + I(t) = 0
	fout(1,1) = e1/R + iL + feval(dobj.f,e1,Is,Vt);
...
function Jf = df_dx(x, DAE)
	...
	%	C de1/dt + e1/R + iL + diode(e1,Is,Vt) + I(t) = 0
	%fout(1,1) = e1/R + iL + feval(dobj.f,e1,Is,Vt);
	[Id, dId_de1] = feval(dobj.f,e1,Is,Vt);
	Jf(1,1) = 1/R + dId_de1;
	Jf(1,2) = 1;
