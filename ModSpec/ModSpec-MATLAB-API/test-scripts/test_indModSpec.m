function ok = test_indModSpec(arg)
%function test_indModSpec() or ('update')
% test-script for indModSpec 
...
if nargin ~= 0 && strcmp(class(arg), 'char') && strcmp(arg, 'update')
	% update
	[filename, is_new] = run_ModSpec_functions(MOD, 'indModSpec', 'update');
	if is_new
		load(filename); % get ref in the workspace
		n_dtests = length(ref.dtests);
		% add more specific dynamic test cases for indModSpec
		% add all zeros
		...
		% add random test cases for indModSpec
		ref.rtests{1} = 1;
		% save ref
		save(filename, 'ref');
	[filename, is_new] = run_ModSpec_functions(MOD, 'indModSpec', 'update');
	...
	% no update
	[filename, is_new, ok] = run_ModSpec_functions(MOD, 'indModSpec');
end
