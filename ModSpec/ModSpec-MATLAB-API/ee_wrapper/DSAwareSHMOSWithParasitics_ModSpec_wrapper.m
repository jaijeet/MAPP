function MOD = DSAwareSHMOSWithParasitics_ModSpec_wrapper()
%function MOD = DSAwareSHMOSWithParasitics_ModSpec_wrapper()
% This function returns a ModSpec model for a Shichman Hodges MOSFET model
% with parasitics capacitors Cgs and Cgd, parasitic resistors Rs and Rd.
% DS inversion is also implemented in the model such that the device's d and s
% terminals are symmetric. 
% The function should be self-explanatory. To see the code:
% >> type DSAwareSHMOSWithParasitics_ModSpec_wrapper;
% or
% >> edit DSAwareSHMOSWithParasitics_ModSpec_wrapper;
%
	MOD = ee_model();
...
function ids = SH_noinv(vgs, vds, Beta, vth, m)
    if m * vgs < vth
        ids = 0;
    else
        if m * (vds - vgs) > -vth
...
function ids = SH(vgs, vds, Beta, vth, Type)
    ...
    if m*vds >= 0
        ids = SH_noinv(vgs, vds, Beta, vth, m);
    else
        ids = -SH_noinv(vgs-vds, -vds, Beta, vth, m);
    end
end
