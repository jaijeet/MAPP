...
%function MOD = RRAM_ModSpec_wrapper(uniqID)
% Note: v1 scales internal unk l in 'nm', simplifies equation for 'low_bias',
%       but is not working
...
function out = fqei(S, forq, eori)
    ...
    if 1 == strcmp(eori,'e') % e
        if 1 == strcmp(forq, 'f') % f
			...
			% c1_l = Alpha * (h - 1e-9*l) / (2*sqrt(Phi0*q));
            % low_bias = 8*pi*m*q*(k*T)^2 / plank^3 * pi/ ...
            %            ( c1_l * k*T * sin(pi*c1_l *k*T) * safeexp(-Alpha * (h-1e-9*l) *sqrt(q*Phi0), maxslope) ...
            %              * mysinh(Alpha*(h-1e-9*l), maxslope) * abs(vpn)/4 * sqrt(q/Phi0));
            % % TODO: properly update vecvalder to support sinh, then change mysinh back

            if abs(vpn) < SMALLNUM
				c1_l = Alpha * (h - 1e-9*l) / (2*sqrt(Phi0*q));
			else
				c1_l = Alpha * (h - 1e-9*l) * (sqrt(Phi0) - sqrt(Phi0 - abs(vpn))) / (abs(vpn)*sqrt(Phi0*q));
			end

            % low_bias = 8*pi^2*m*q*(k*T) / plank^3 / (c1_l * sin(pi*c1_l * k * T));
            low_bias = 8*pi^2*m*q*(k*T) / plank^3 / (c1_l * (pi*c1_l * k * T)); %TODO: change back

            out(1,1) = sign(vpn) * Area * ( ...
                       (1-smoothstep(abs(vpn)-Phi0, smoothing)) * low_bias + ...
                       smoothstep(abs(vpn)-Phi0, smoothing)     * high_bias);
            out(1,1) = out(1,1) + vpn * 1e-12;
        else % q
            out(1,1) = 0;
        end % forq
    else % i
        if 1 == strcmp(forq, 'f') % f
            Vt = k*T/q;
            ion_flow = Beta * 1e9 * safeexp(-Ua/Vt, maxslope) * mysinh(vpn * Kappa/T, maxslope);
