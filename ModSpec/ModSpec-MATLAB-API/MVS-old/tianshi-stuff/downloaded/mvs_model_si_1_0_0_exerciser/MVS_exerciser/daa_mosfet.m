...
for len_bias=1:length(Vd_pre)
    ...
    Vds=abs(Vd-Vs);
    Vgs=max(tipe*(Vg-Vs),tipe*(Vg-Vd));
    Vbs=max(tipe*(Vb-Vs),tipe*(Vb-Vd));
    ...
    Fsatq=(abs(Vdsi)./Vdsatq)./((1+(abs(Vdsi)./Vdsatq).^beta).^(1./beta));
    x=1-Fsatq;
    den=15*(1+x).^2;
    ...
    Qb(len_bias,1)=-tipe*W*Leff*(Cg*gamma*sqrt(psis-Vbsi)+(a-1)./a.*Qinv.*(1-qi));
    ...
    Vt0x=Vt0+gamma*(sqrt(phib-tipe*(Vb-Vsint))-sqrt(phib));
    Vt0y=Vt0+gamma*(sqrt(phib-tipe*(Vb-Vdint))-sqrt(phib));
