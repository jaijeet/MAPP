function MOD = MOS1ModSpec_v6_wrapper(uniqID)
%function MOD = MOS1ModSpec_v6_wrapper(uniqID)
%
% v6: moved from v5,
%     SMOOTHING is added and safesqrt() are used in the calculation of VTH to
%     get rid of imaginary parts resulted from sqrt()
%
% v5: moved from v4,
%     diode charges are considered
%     diode currents are considered
%     Qgd, Qgs, Qgb are considered
%     DS inversion is considered
%     P type is considered
%     no Rs, Rd
%     no init/limiting
%
% v4: moved from v3,
...
%   - 'SMOOTHING' 1e-12 NONE 'smoothing factor'
...
%2014/06/27: Tianshi Wang <tianshi@berkeley.edu> Created
    ...
    MOD = add_to_ee_model (MOD, 'parms', {'SMOOTHING', 1e-12});
    % element parms:
...
function out = fqei(S, forq, eori)
	...
	if strcmp(TYPE, 'N') || strcmp(TYPE, 'n')
		if vds >=0 
			VTH = MOS1_VTH(vbs, VTO, GAMMA, PHI, SMOOTHING);
			ids = forward_ids(vds, vgs, VTH, KP, W, L, LD, LAMBDA);
		else % vds < 0, drain-source inversion
			VTH = MOS1_VTH(vbd, VTO, GAMMA, PHI, SMOOTHING);
			ids = -forward_ids(-vds, vgd, VTH, KP, W, L, LD, LAMBDA);
		end
	elseif strcmp(TYPE, 'P') || strcmp(TYPE, 'p')
		if vds <=0 
			VTH = MOS1_VTH(-vbs, -VTO, GAMMA, PHI, SMOOTHING);
			ids = -forward_ids(-vds, -vgs, VTH, KP, W, L, LD, LAMBDA);
		else
		...
		error('Unexpected MOSFET Type, should be ''N'' or ''P''');
	end
    ...
    %}
	if strcmp(TYPE, 'N') || strcmp(TYPE, 'n')
		% TODO: calculation of ibd, ibs needs better modularity
		if vbs <= 0
			gbs = IS/vt + GMIN;
			ibs = gbs*vbs;
		else
		...
		% 	ibd = IS * (evbd-1) + GMIN*vbd;
		% end
		%TODO: commented out to debug diode currents in P type
		ibd = 0;
		ibs = 0;
	end
    ...
    if 1 == strcmp(forq, 'q') % calculate charges
        ...
        Leff = L - 2*LD;
        oxideCapFactor = 3.9 * 8.854214871e-12/TOX;
        OxideCap = oxideCapFactor * Leff * W;
		%
        tPhi = PHI; % simplified when temp == tnom
		%
        arg = 1 - FC;
        sarg = exp((-MJ) * log(arg));
        tVbi = VTO - intTYPE * (GAMMA * sqrt(PHI));
        Von = (tVbi * intTYPE) + GAMMA*sarg;
		%
		if vds >=0 
			vgst = vgs - Von;
		else % vds < 0, drain-source inversion
		...
		if strcmp(TYPE, 'N') || strcmp(TYPE, 'n')
			if vds >=0 
				...
				[capgd, capgs, capgb] = qmeyer(vgd, vgs, vgb, Von, Vdsat, tPhi, OxideCap);
			end
		else % P-type
			% TODO: not so sure...
			if vds <=0 
        ...
        Leff = L - 2*LD;
        GateSourceOverlapCap = CGSO * W;
        GateDrainOverlapCap = CGDO * W;
        GateBulkOverlapCap = CGBO * Leff;
		...
		qgd = Capgd * vgd;
		qgb = Capgb * vgb;
        % done with GD, GS, GB charges calculation
        % ======================================================================
    end
...
function VTH = MOS1_VTH(vbs, VTO, GAMMA, PHI, SMOOTHING)
%function VTH = MOS1_VTH(vbs, VTO, GAMMA, PHI, SMOOTHING)
...
%
% SMOOTHING is added to prevent sqrt to generate imaginary parts during
% first iter of NR, where vbs can be virtually any value.
    VTH = VTO + GAMMA * (safesqrt(2*PHI-vbs, SMOOTHING) - sqrt(2*PHI));
end % MOS1_VTH
