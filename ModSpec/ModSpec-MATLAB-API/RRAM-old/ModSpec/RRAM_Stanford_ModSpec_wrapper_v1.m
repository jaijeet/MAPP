...
function out = fqei(S, forq, eori)
	...
	Gap_ddt = - Vel0 * exp(- q * Ea / kb / T_cur) * mysinh(gamma * a0 / tox * q * vtb / kb / T_cur, maxslope);
	...
	Itb = I0 * safeexp(-Gap/g0, maxslope) * mysinh(vtb/V0, maxslope);

    Itb = Itb - 1 * Gap; %TODO: gmin hack
    Itb = Itb + 1e-8 * vtb; %TODO: gmin hack
