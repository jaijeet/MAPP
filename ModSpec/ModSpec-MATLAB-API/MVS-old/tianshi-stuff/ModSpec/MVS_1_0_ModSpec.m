function MOD = MVS_1_0_ModSpec(uniqID)
...
function [fe, qe, fi, qi] = fqei_all(vecX, vecY, vecLim, vecU, flag, MOD)
	...
	% vdb = vecX(1); vgb = vecX(2); vsb = vecX(2);
	% vdib = vecY(1); vsib = vecY(2);
...
function [iddi, idisi, isis, qgb, qdib, qsib] = daa_mosfet_core_model(vd, vg, vs, vb, vdi, vsi, MOD)
	...
	Vgsraw  = tipe*(vgsi);
	Vgdraw  = tipe*(vgdi);
	if (Vgsraw >= Vgdraw) %TODO: commented by Tianshi
	% if 0
		Vds = tipe*(vds); 
		Vgs = tipe*(vgs);
		Vgd = tipe*(vgd);
	...
	Vtpcorr = Vt0+parm_gamma*(safesqrt(smoothabs(phib-Vbs, smoothing), smoothing)-sqrt(phib))-Vdsi*delta; 
	eVgpre = limexp((Vgs-Vtpcorr)/(aphit*1.5)); 
	FFpre  = 1.0/(1.0+eVgpre);
	ab  = 2*(1-0.99*FFpre)*phit;  
	Vcorr  = (1.0+2.0*delta)*(ab/2.0)*(limexp(-Vdsi/ab)); 
	Vgscorr  = Vgsi+Vcorr; 
	Vbscorr  = Vbsi+Vcorr; 
	Vt0bs  = Vt0+parm_gamma*(safesqrt(smoothabs(phib-Vbscorr, smoothing), smoothing)-sqrt(phib)); 
	Vt0bs0  = Vt0+parm_gamma*(safesqrt(smoothabs(phib-Vbsi, smoothing), smoothing)-sqrt(phib)); 
	Vtp  = Vt0bs-Vdsi*delta-0.5*aphit; 
	Vtp0  = Vt0bs0-Vdsi*delta-0.5*aphit; 
	eVg  = limexp((Vgscorr-Vtp)/(aphit)); 
	FF  = 1.0/(1.0+eVg);
	eVg0  = limexp((Vgsi-Vtp0)/(aphit)); 
	FF0  = 1.0/(1.0+eVg0);
	Qref  = Cg*nphit; 
	eta  = (Vgscorr-(Vt0bs-Vdsi*delta-FF*aphit))/(nphit); 
	eta0  = (Vgsi-(Vt0bs0-Vdsi*delta-FFpre*aphit))/(nphit);
	if (eta  <= LARGE_VALUE)
		Qinv_corr = Qref * log(1.0 + limexp(eta));
	else
		Qinv_corr = Qref*eta;
	end
	if (eta0 <= LARGE_VALUE)
		Qinv  = Qref*log(1.0+limexp(eta0));
	else
		Qinv  = Qref*eta0;
	end
	vx0  = vxo; 
	...
	if (parm_gamma == 0)
		a = 1.0;
		if (eta0 <= LARGE_VALUE)
			psis = phib+phit*(1.0+log(log(1.0+SMALL_VALUE+limexp(eta0))));
		else
			psis  = phib+phit*(1.0+log(eta0));
		end
	else
		a  = 1.0+parm_gamma/(2.0*safesqrt(smoothabs(psis-(Vbsi), smoothing), smoothing));
		if (eta0 <= LARGE_VALUE)
			psis  = phib+(1.0-parm_gamma)/(1.0+parm_gamma)*phit*(1.0+log(log(1.0+SMALL_VALUE+limexp(eta0))));
		else 
			psis  = phib+(1.0-parm_gamma)/(1.0+parm_gamma)*phit*(1.0+log(eta0));
		end
	end
	Vgta  = Vgt/a; Vdsatq  = safesqrt(FF0*aphit*aphit+Vgta*Vgta, smoothing);
	...
	if (Vdsi <= tol)
		...
		kq = safesqrt(2.0*P_Q/me*Vdsi, smoothing)/vx0*100.0;
		kq2 = kq*kq;
		qsb = Qinv*(asinh(kq)/kq-(sqrt(kq2+1.0)-1.0)/kq2);
		qdb = Qinv*((safesqrt(kq2+1.0, smoothing)-1.0)/kq2);
	end
	if (CTM_select == 1)
		qs = qsc;
		qd = qdc;
	...
	Qb = -tipe*W*Leff*(Cg*parm_gamma*safesqrt(smoothabs(psis-Vbsi, smoothing), smoothing)+(a-1.0)/(1.0*a)*Qinv*(1.0-qi));
	etai = (Vgsi-(Vt0bs0-FF*aphit))/(nphit);
	if (etai <= LARGE_VALUE)
		Qinvi = Qref*log(1.0+limexp(etai));
	else
		Qinvi = Qref*etai;
	end
	dQinv  = Qinv-Qinvi;
	...
	Qsov  = Cofs*(vgsi);
	Qdov  = Cofd*(vgdi);
	Vt0x  = Vt0+parm_gamma*(safesqrt(smoothabs(phib-tipe*(vbsi), smoothing), smoothing)-sqrt(phib));
	Vt0y  = Vt0+parm_gamma*(safesqrt(smoothabs(phib-tipe*(vbdi), smoothing), smoothing)-sqrt(phib));
