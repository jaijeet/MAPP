function ok = test_circadianOsc_Drosophila(arg)
% test-script for circadianOsc_Drosophila 
% Author: Tianshi Wang 2012-11-19

DAE = circadianOsc_Drosophila();

if nargin ~= 0 && strcmp(class(arg), 'char') && strcmp(arg, 'update')
	% update
	[filename, is_new] = run_DAEAPI_functions(DAE, 'circadianOsc_Drosophila', 'update');
	if is_new
		load(filename); % get ref in the workspace
		n_dtests = length(ref.dtests);
		% add more specific dynamic test cases for circadianOsc_Drosophila
		% add all zeros
		...
		% add random test cases for circadianOsc_Drosophila
		ref.rtests{1} = 1;
		% save ref
		save(filename, 'ref');
	[filename, is_new] = run_DAEAPI_functions(DAE, 'circadianOsc_Drosophila', 'update');
	...
	% no update
	[filename, is_new, ok] = run_DAEAPI_functions(DAE, 'circadianOsc_Drosophila');
end
