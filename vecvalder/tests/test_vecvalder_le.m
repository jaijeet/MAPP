function [ok, funcname] = test_vecvalder_le(n, m, tol)
%function [ok, funcname] = test_vecvalder_le(n, m, tol)
%
%run tests on vecvalder/le; compare with handcoded expectations.
%
%Both arguments are optional:
%   n: size of vecvalder vector to test (defaules to 10 if unspecified)
%   m: number of indep vars for derivatives (defaules to n+1 if unspecified)
%   tol: absolute tolerance for error check (defaules to 0 if unspecified)
%
%Return values:
%   ok: 1 if everything passes, 0 otherwise.
%   funcname: always set to 'le()'
%
%This function tests:
