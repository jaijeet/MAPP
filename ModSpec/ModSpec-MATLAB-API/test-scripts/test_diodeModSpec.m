function ok = test_diodeModSpec(arg)
%function test_diodeModSpec() or ('update')
% test-script for diodeModSpec 
...
if nargin ~= 0 && strcmp(class(arg), 'char') && strcmp(arg, 'update')
    % update
    [filename, is_new] = run_ModSpec_functions(MOD, 'diodeModSpec', 'update');
    if is_new
        load(filename); % get ref in the workspace
        n_dtests = length(ref.dtests);
        % add more specific dynamic test cases for diodeModSpec
        % add all zeros
        ...
        % add random test cases for diodeModSpec
        ref.rtests{1} = 1;
        % save ref
        save(filename, 'ref');
    [filename, is_new] = run_ModSpec_functions(MOD, 'diodeModSpec', 'update');
    ...
    % no update
    [filename, is_new, ok] = run_ModSpec_functions(MOD, 'diodeModSpec');
end
