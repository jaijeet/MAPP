function cktnetlist = BJT_PNP_char_curves_ckt()
%function cktnetlist = BJT_PNP_char_curves_ckt()
%This function returns a cktnetlist structure for a circuit that generates
%characteristic curves of Ebers Moll PNP BJTs.
%
%The circuit
%   An PNP-type BJT (Ebers Moll model) driven by VBE and VCE voltages sources
%   to generate characteristic curves
%
%To see the schematic of this circuit, run:
%
% showimage('BJT_PNP_char_curves.jpg'); %TODO
%
%Examples
%--------
%
% DAE =  MNA_EqnEngine(BJT_PNP_char_curves_ckt());
% 
...
% xlabel 'VCE';
% ylabel 'IC';
% title 'Ebers Moll BJT (PNP) characteristic curves';
% hold on;
%
% for c = 1:nVBEs
% 	DAE = feval(DAE.set_uQSS, 'Vbe:::E', VBEs(c), DAE);
% 	swp = dot_dcsweep(DAE, [], 'Vce:::E', min(VCEs), max(VCEs), nVCEs);
