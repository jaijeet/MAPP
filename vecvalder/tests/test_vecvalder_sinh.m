function [ok, funcname] = test_vecvalder_sinh(n, m, tol)
%function [ok, funcname] = test_vecvalder_sinh(n, m, tol)
%
%run tests on vecvalder/sinh; compare with handcoded expectations.
%
...
%   funcname: always set to 'sinh()'
%
%This function tests:
%   sinh(vecvalder_n_by_m)
%
...
%Author: Tianshi Wang, 2016/01/18
    ...
    % test sinh(vecvalder_n_by_m)
    oof = rand(n,1); oof2 = rand(n, m);
    u = vecvalder(oof, oof2);
    h = sinh(u);
    hv = val(h); hJ = jac(h);
    % if h(u) = sinh(u), dh_dx = cosh(u)*du_dx
    expected_hv = sinh(oof);
    expected_hJ = (cosh(oof)*ones(1,m)).*oof2;
