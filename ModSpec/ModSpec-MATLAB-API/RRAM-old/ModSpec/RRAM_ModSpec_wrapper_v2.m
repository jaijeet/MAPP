...
function out = fqei(S, forq, eori)
    ...
    if 1 == strcmp(eori,'e') % e
        ...
        if 1 == strcmp(forq, 'f') % f
			...
			clip_ge_h = smoothstep(l-1e9*h, smoothing);
			clip_le_0 = 1 - smoothstep(l, smoothing);

            out(1,1) = 1e9 * ion_flow + (-exp(100*(l-5))) * clip_ge_h + (exp(-100*l)) * clip_le_0;
            % TODO: 100 is hard-coded, and is too large
