function ok = test_ccvsModSpec(arg)
%function test_ccvsModSpec() or ('update')
% test-script for ccvsModSpec 
...
if nargin ~= 0 && strcmp(class(arg), 'char') && strcmp(arg, 'update')
    % update
    [filename, is_new] = run_ModSpec_functions(MOD, 'ccvsModSpec', 'update');
    if is_new
        load(filename); % get ref in the workspace
        n_dtests = length(ref.dtests);
        % add more specific dynamic test cases for ccvsModSpec
        % add all zeros
        ...
        % add random test cases for ccvsModSpec
        ref.rtests{1} = 1;
        % save ref
        save(filename, 'ref');
    [filename, is_new] = run_ModSpec_functions(MOD, 'ccvsModSpec', 'update');
    ...
    % no update
    [filename, is_new, ok] = run_ModSpec_functions(MOD, 'ccvsModSpec');
end
