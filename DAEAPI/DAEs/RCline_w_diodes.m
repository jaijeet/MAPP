function DAE = RCline_w_diodes(uniqIDstr,nsegs,lineR,lineC, Is, Vt)  
%function DAE = RCline_w_diodes(uniqIDstr,nsegs, lineR, lineC, Is, Vt) 
% RC line circuit
%Authors: 
% the circuit: 
...
%           - an ideal exponential diode from i to ground, pointing that way
%           - another ideal exponential diode from ground to i
%             - implemented using diode.m
%	there is one output: the voltage and node nseg
%	
% unknowns are: x = [e1, ..., enseg]^T
% circuit equations are:
%	segment 1: (add to node 1 KCL) [e1-u(t)]/R + C de1/dt + 2kT/R n_1(t) +
%	                              diode(e1, Is, Vt) - diode(-e1, Is, Vt)
%	segment i, 1<i<=nsegs:
%		add to node i-1 KCL: (eim1-ei)/R - 2kT/R n_i(t)
%		add to node i KCL: (ei-eim1)/R + C dei/dt + 2kT/R n_i(t) + ...
%                                 diode(ei, Is, Vt) - diode(-ei, Is, Vt)
%
...
% version, help string: 
	...
	DAE.Usage = help('RCline_w_diodes');
	if nargin < 1
		DAE.uniqIDstr = '';
	else
...
function fout = f(x, DAE)
	fout = DAE.Gmat * x;
    diodecurrentsDown = feval(DAE.d_struct.f, x, DAE.Is, DAE.Vt);
    diodecurrentsUp = feval(DAE.d_struct.f, -x, DAE.Is, DAE.Vt);
    fout = fout + diodecurrentsDown - diodecurrentsUp;
end % f(...)
...
function Jf = df_dx(x, DAE)
	Jf = DAE.Gmat;
    [dummy, diodeJacTermsDown] = feval(DAE.d_struct.f, x, DAE.Is, DAE.Vt);
    [dummy, diodeJacTermsUp] = feval(DAE.d_struct.f, -x, DAE.Is, DAE.Vt);
    Jf = Jf + diag(diodeJacTermsUp + diodeJacTermsDown);
end % df_dx(...)
