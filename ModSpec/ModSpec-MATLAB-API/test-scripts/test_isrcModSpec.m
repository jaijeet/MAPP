function ok = test_isrcModSpec(arg)
%function test_isrcModSpec() or ('update')
% test-script for isrcModSpec 
...
if nargin ~= 0 && strcmp(class(arg), 'char') && strcmp(arg, 'update')
    % update
    [filename, is_new] = run_ModSpec_functions(MOD, 'isrcModSpec', 'update');
    if is_new
        load(filename); % get ref in the workspace
        n_dtests = length(ref.dtests);
        % add more specific dynamic test cases for isrcModSpec
        % add all zeros
        ...
        % add random test cases for isrcModSpec
        ref.rtests{1} = 1;
        % save ref
        save(filename, 'ref');
    [filename, is_new] = run_ModSpec_functions(MOD, 'isrcModSpec', 'update');
    ...
    % no update
    [filename, is_new, ok] = run_ModSpec_functions(MOD, 'isrcModSpec');
end
