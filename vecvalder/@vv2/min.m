function h = min(u,v)
%function h = min(u,v)
%VECVALDER/MIN (vv2 version) overloads MIN if at least one argument is a
%vecvalder.
%
%NO SIZE CHECKS (for efficiency): relying on numeric min to catch size errors.
%
%Note: min(u,v) = 0.5*(-abs(u-v) + v + u), but for efficiency, this
...
%Author: JR, 2014/06/18
  ...
  elseif ~isobject(v) %v is a scalar
    ...
    h = u; % avoid using constructor
    h.valder = [min(u.valder(:,1),v.valder(:,1)), ...
