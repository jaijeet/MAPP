function MOD = MOS1ModSpec_v5_wrapper(uniqID)
%function MOD = MOS1ModSpec_v5_wrapper(uniqID)
%
% v5: moved from v4,
...
function out = fqei(S, forq, eori)
	...
	if strcmp(TYPE, 'N') || strcmp(TYPE, 'n')
		if vds >=0 
			...
			VTH = MOS1_VTH(vbd, VTO, GAMMA, PHI);
			ids = -forward_ids(-vds, vgd, VTH, KP, W, L, LD, LAMBDA);
		end
	elseif strcmp(TYPE, 'P') || strcmp(TYPE, 'p')
		if vds <=0 
			VTH = MOS1_VTH(-vbs, -VTO, GAMMA, PHI); % TODO: confirm this
			ids = -forward_ids(-vds, -vgs, VTH, KP, W, L, LD, LAMBDA);
		else
			VTH = MOS1_VTH(-vbd, -VTO, GAMMA, PHI); % TODO: confirm this
			ids = forward_ids(vds, -vgd, VTH, KP, W, L, LD, LAMBDA);
		end
	else
...
% is the threshold voltage in the presence of back-gate bias, vbs < 0
    % VTH = VTO + GAMMA * (sqrt(2*PHI-vbs) - sqrt(2*PHI));
    VTH = VTO + GAMMA * (sqrt(2*PHI+abs(vbs)) - sqrt(2*PHI));
end % MOS1_VTH
