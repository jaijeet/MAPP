function DAE = parallelRLC(uniqIDstr) 
%function DAE = parallelRLC(uniqIDstr)   
% Current source in parallel with parallel RLC
%author: J. Roychowdhury, 2009/11/08; updates 2011/11/05
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% The Circuit:
% 
% [ I(t) || R || L || C  from node n1 to gnd
%
% unknowns are: e1, iL
% MNA equations are:
%
...
function out = daename(DAE)
	out = sprintf('parallelRLC: I(t) || R || L || C');
% end daename()
...
function parmvals = parmdefaults(DAE)
	parmvals = {10000, 1e-6, 1e-9}; % [R, C, L] = deal(DAE.parms{:});
% end parmdefaults(...)
