function [ok, funcname] = test_vecvalder_ne(n, m, tol)
%function [ok, funcname] = test_vecvalder_ne(n, m, tol)
%
%run tests on vecvalder/ne; compare with handcoded expectations.
%
...
%Author: JR, 2014/06/18
    ...
    h = (u ~= v);
    expected_h = ~prod([oof == poof, oof2==poof2]*1, 2);
    ok = ok && (abs(sum(h-expected_h)) <= tol); 
    h = (u ~= u);
    expected_h = ~prod([oof == oof, oof2==oof2]*1, 2);
    ok = ok && (abs(sum(h-expected_h)) <= tol); 
    if ~(1==ok) 
        return; 
    end
