...
% (RRAM) cell.
%
% verion 5: copied from v4, try to figure our the convergence problem when
%           vpn < Phi0.
% verion 4: copied from v3, scaled l and eqn3
%           but when vpn=0, ipn~=0, this problem is showing up.
    ...
    if 1 == strcmp(eori,'e') % e
        if 1 == strcmp(forq, 'f') % f
            ...
            if 1e-9*l < h - SMALLNUM2 * h;
				...
				low_bias = slope * (1e-9*l - (h - SMALLNUM2 * h)) + C/(h*SMALLNUM2)^2;
            end
