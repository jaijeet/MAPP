function ok = test_vccsModSpec(arg)
%function test_vccsModSpec() or ('update')
% test-script for vccsModSpec 
...
if nargin ~= 0 && strcmp(class(arg), 'char') && strcmp(arg, 'update')
    % update
    [filename, is_new] = run_ModSpec_functions(MOD, 'vccsModSpec', 'update');
    if is_new
        load(filename); % get ref in the workspace
        n_dtests = length(ref.dtests);
        % add more specific dynamic test cases for vccsModSpec
        % add all zeros
        ...
        % add random test cases for vccsModSpec
        ref.rtests{1} = 1;
        % save ref
        save(filename, 'ref');
    [filename, is_new] = run_ModSpec_functions(MOD, 'vccsModSpec', 'update');
    ...
    % no update
    [filename, is_new, ok] = run_ModSpec_functions(MOD, 'vccsModSpec');
end
