function ok = test_diodeCapIsrc_daeAPIv6(arg)
% test-script for diodeCapIsrc_daeAPIv6() or ('update') 
% Author: Tianshi Wang 2012-11-19

DAE = diodeCapIsrc_daeAPIv6();

if nargin ~= 0 && strcmp(class(arg), 'char') && strcmp(arg, 'update')
	% update
	[filename, is_new] = run_DAEAPI_functions(DAE, 'diodeCapIsrc_daeAPIv6', 'update');
	if is_new
		load(filename); % get ref in the workspace
		n_dtests = length(ref.dtests);
		% add more specific dynamic test cases for diodeCapIsrc_daeAPIv6
		% add all zeros
		...
		% add random test cases for diodeCapIsrc_daeAPIv6
		ref.rtests{1} = 1;
		% save ref
		save(filename, 'ref');
	[filename, is_new] = run_DAEAPI_functions(DAE, 'diodeCapIsrc_daeAPIv6', 'update');
	...
	% no update
	[filename, is_new, ok] = run_DAEAPI_functions(DAE, 'diodeCapIsrc_daeAPIv6');
end
